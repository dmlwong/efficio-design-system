...
description: Core design principles, visual philosophy, and usage rules (do's and don'ts) for Orbit, Efficio's procurement-intelligence design system. Use whenever building, reviewing, or discussing any Orbit or Efficio app UI and you need the "why": how Orbit handles colour hierarchy and the single primary action, typography and density, elevation/depth (mostly-flat, tonal layering over shadow), shape language, and marking AI-authored surfaces distinctly. Orbit is light-only — no dark mode. Consult this before the other orbit-* skills for any non-trivial Orbit design decision. For exact token values use orbit-tokens; for component APIs use orbit-components; for accessibility contracts use orbit-a11y.
---
