...
import type { ComponentDoc } from './componentDocs';

const SECTION_SCROLL_MARGIN_TOP = 96;
const SelectedSectionContext = React.createContext<string | null>(null);
...
function DocSection({
  ...
  const selectedSectionId = React.useContext(SelectedSectionContext);

  if (selectedSectionId && selectedSectionId !== id) {
    return null;
  }
...
function DocumentationList({ title, items }: { title: string; items: string[] }) {
  return (
    <section>
      <Headings size="Heading 5">{title}</Headings>
      <ul
        style={{
          margin: 'var(--orbit-space-s) var(--orbit-space-none) var(--orbit-space-none)',
...
function ComponentDocumentation({ doc }: { doc: ComponentDoc }) {
  const importSnippet = `import { ${doc.imports.join(', ')} } from '@efficio/orbit';`;
  ...
  return (
    <section
      style={{
        ...
        marginBottom: 'var(--orbit-space-xxl)',
        paddingBottom: 'var(--orbit-space-xxl)',
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--orbit-space-s)' }}>
        ...
        <Headings size="Heading 3">Documentation</Headings>
        <Text variant="Secondary">{doc.description}</Text>
      </div>
      ...
      >
        <DocumentationList title="When To Use" items={doc.whenToUse} />
        <DocumentationList title="Guidance" items={doc.guidelines} />
        <DocumentationList title="Examples Covered" items={doc.examples} />
        <DocumentationList title="Relevant Tokens" items={doc.tokens} />
      </div>
      ...
      >
        <section>
          <Headings size="Heading 5">Import</Headings>
          <pre
            style={{
              margin: 'var(--orbit-space-s) var(--orbit-space-none) var(--orbit-space-none)',
        ...
        <section>
          <Headings size="Heading 5">Source</Headings>
          <ul
            style={{
              margin: 'var(--orbit-space-s) var(--orbit-space-none) var(--orbit-space-none)',
              paddingInlineStart: 'var(--orbit-space-base)',
...
export function ComponentShowcase({
  ...
  const [selectedToggleCard, setSelectedToggleCard] = useState<string | null>(null);

  return (
    <SelectedSectionContext.Provider value={selectedSectionId ?? null}>
        {componentDoc && <ComponentDocumentation doc={componentDoc} />}
        ...
        {/* Card */}
        <DocSection id="card" title="Card" description="Figma-parity content container with explicit shadow, padding, and indicator rail variants.">
          <Row label="ContentPlaceHolder Padding">
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {(['Small', 'Base', 'Medium'] as const).map((pad) => (
                <Card key={pad} style={{ width: 200 }}>
                  <CardContent padding={pad}>
                    <Headings size="Heading 5">Padding: {pad}</Headings>
                    <p style={{ fontSize: 12, color: 'var(--orbit-color-text-secondary)', marginTop: 4 }}>Figma slot</p>
                  </CardContent>
                </Card>
              ))}
            </div>
          ...
          <Row label="HasShadow">
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {[false, true].map((hasShadow) => (
                <Card key={String(hasShadow)} hasShadow={hasShadow} style={{ width: 200 }}>
                  <CardContent>
                    <Headings size="Heading 5">HasShadow={String(hasShadow)}</Headings>
                    <p style={{ fontSize: 12, color: 'var(--orbit-color-text-secondary)', marginTop: 4 }}>Figma variant</p>
                  </CardContent>
                </Card>
              ))}
            </div>
          </Row>
          <Row label="State Variants">
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {CARD_FIGMA_VARIANTS.map(({ state, hasShadow, label }) => (
                <Card key={`${state}-${label}`} state={state} hasShadow={hasShadow} style={{ width: 180 }}>
                  <CardContent>
          ...
          <Row label="Has Highlight / Indicator">
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              <Card state="Information" style={{ width: 200 }}>
                <CardContent>
                  <Headings size="Heading 5">Off</Headings>
                  <p style={{ fontSize: 12, color: 'var(--orbit-color-text-secondary)', marginTop: 4 }}>indicator=false</p>
                </CardContent>
              </Card>
              <Card state="Information" indicator style={{ width: 200 }}>
                <CardContent>
                  <Headings size="Heading 5">On</Headings>
                  <p style={{ fontSize: 12, color: 'var(--orbit-color-text-secondary)', marginTop: 4 }}>indicator=true</p>
                </CardContent>
              </Card>
            </div>
          </Row>
        ...
        {/* Toggle Card */}
        <DocSection id="toggle-card" title="Toggle Card" description="Interactive card that acts as a selection control with four distinct states.">
          <Row label="All States">
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {(['Default', 'Hover', 'Selected', 'Disabled'] as const).map((s) => (
                <ToggleCard key={s} status={s} style={{ width: 200 }}>
                  <div style={{ padding: 'var(--orbit-space-base)' }}>
                    <Headings size="Heading 5">{s}</Headings>
                    <p style={{ fontSize: 12, color: 'var(--orbit-color-text-secondary)', marginTop: 4 }}>
                      Status: {s}
                    </p>
                  </div>
          ...
          <Row label="Interactive Selection Group (click to select)">
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {['Basic Plan', 'Standard Plan', 'Enterprise Plan'].map((plan) => (
                <ToggleCard
                  key={plan}
                  status={selectedToggleCard === plan ? 'Selected' : 'Default'}
                  onClick={() => setSelectedToggleCard(plan)}
                  style={{ width: 200 }}
                >
                  <div style={{ padding: 'var(--orbit-space-base)' }}>
                    <Headings size="Heading 5">{plan}</Headings>
                    <p style={{ fontSize: 12, color: 'var(--orbit-color-text-secondary)', marginTop: 4 }}>
                      Click to select
                    </p>
                  </div>
        ...
        <DocSection id="chip" title="Chip" description="Compact label for categorisation, status, or metadata.">
          <Row label="High-Contrast Chips">
            <Chip variant="Information" contrast="High" label="Label" />
            <Chip variant="Success" contrast="High" label="Label" />
            <Chip variant="Warning" contrast="High" label="Label" />
            <Chip variant="Error" contrast="High" label="Label" />
            <Chip variant="Style 1" contrast="High" label="Label" />
            <Chip variant="None" contrast="High" label="Label" />
            <Chip variant="Disabled" contrast="High" label="Label" />
          </Row>
          <Row label="Low-Contrast Chips">
            ...
            <Chip variant="Style 1" label="Label" />
            <Chip variant="Style 2" label="Label" />
            <Chip variant="Style 3" label="Label" />
            <Chip variant="Style 4" label="Label" />
            <Chip variant="None" label="Label" />
            <Chip variant="Outline" label="Label" />
            <Chip variant="Disabled" label="Label" />
          </Row>
          <Row label="Mini Chips">
            <Chip size="Mini" variant="Information" contrast="High" label="Label" />
            <Chip size="Mini" variant="Success" contrast="High" label="Label" />
            <Chip size="Mini" variant="Warning" contrast="High" label="Label" />
            <Chip size="Mini" variant="Error" contrast="High" label="Label" />
            <Chip size="Mini" variant="Style 1" contrast="High" label="Label" />
            <Chip size="Mini" variant="Information" label="Label" />
            <Chip size="Mini" variant="Style 2" label="Label" />
            <Chip size="Mini" variant="Style 3" label="Label" />
            <Chip size="Mini" variant="Style 4" label="Label" />
            <Chip size="Mini" variant="None" label="Label" />
        ...
        {/* CP Workspace Shell */}
        <DocSection id="cp-workspace-shell" title="CP Workspace Shell" description="Connected Platform workspace shell with Live-matched rail, headers, navigation, and milestone timeline.">
          <Row label="Full shell">
            <div
              style={{
                width: '100%',
                height: 720,
          ...
          <Row label="Rail">
            <div style={{ height: 360, overflow: 'hidden', border: '1px solid var(--orbit-color-border-default)' }}>
              <CpSideRail
                items={CP_RAIL_ITEMS}
                user={{ name: 'Derek Wong', initials: 'DW' }}
                logoLabel="DEV"
          ...
          <Row label="Breadcrumb header">
            <div style={{ width: '100%', border: '1px solid var(--orbit-color-border-default)' }}>
              <CpBreadcrumbHeader items={CP_BREADCRUMB_ITEMS} />
            </div>
          </Row>

          <Row label="Primary header">
            <div style={{ width: '100%', border: '1px solid var(--orbit-color-border-default)' }}>
              <CpWorkspaceHeader
                workspaceTitle="Connected Platform - Development Workstreams"
                primaryNavItems={CP_PRIMARY_NAV_ITEMS}
          ...
          <Row label="Primary nav only">
            <CpPrimaryNav items={CP_PRIMARY_NAV_ITEMS} />
          </Row>

          <Row label="Secondary nav">
            <div style={{ width: '100%' }}>
              <CpSecondaryNav
