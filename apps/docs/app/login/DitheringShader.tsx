...
type DitheringShape = keyof typeof DITHERING_SHAPES;
type DitheringType = keyof typeof DITHERING_TYPES;
...
function parseCssColor(value: string, alpha = 1): Rgba | null {
  ...
  return [
    ...
    Math.max(0, Math.min(1, parts[3] ?? alpha)),
  ];
}
...
function resolveCssVariableColor(
  ...
  probe.remove();
  return parseCssColor(color, alpha);
}
...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const background = resolveCssVariableColor(wrapper, backgroundVar);
    const foreground = resolveCssVariableColor(wrapper, foregroundVar, foregroundAlpha);

    if (!gl || !background || !foreground) return undefined;
    ...
    const render = () => {
      ...
      const currentTime = ((window.performance.now() - startedAt) * 0.001) * effectiveSpeed;
      gl.clear(gl.COLOR_BUFFER_BIT);
      gl.useProgram(program);

      if (uniformLocations.u_time) gl.uniform1f(uniformLocations.u_time, currentTime);
      if (uniformLocations.u_resolution) {
        gl.uniform2f(uniformLocations.u_resolution, canvas.width, canvas.height);
      }
      if (uniformLocations.u_colorBack) gl.uniform4fv(uniformLocations.u_colorBack, background);
      if (uniformLocations.u_colorFront) gl.uniform4fv(uniformLocations.u_colorFront, foreground);
      if (uniformLocations.u_shape) gl.uniform1f(uniformLocations.u_shape, DITHERING_SHAPES[shape]);
      if (uniformLocations.u_type) gl.uniform1f(uniformLocations.u_type, DITHERING_TYPES[type]);
      if (uniformLocations.u_pxSize) gl.uniform1f(uniformLocations.u_pxSize, pxSize);
  ...
  }, [backgroundVar, foregroundAlpha, foregroundVar, pxSize, shape, speed, type]);
